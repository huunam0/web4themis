<?php
session_start();
//if (isset($_SESSION['tuser'])) header("Location: index.php?kt=".$_SESSION['contest']);
if (isset($_SESSION['tuser'])) header("Location: index.php");
include("config.php");  //de lay thu muc contest
//include($contestDir . "info.php");
if (isset($_POST['username'])) {
	$username = $_POST['username'];
	$password = $_POST['password'];
	$contest = $_POST['contest'];
	//echo $contestDir . $contest . "/account.xml";
	$dom = new DOMDocument();
    $dom->load($contestDir . $contest . "/account.xml");
	...
	//if ($found == 1) header("Location: index.php?kt=$contest");
  ...
  <body>
	...
	<center><h2></h2></center>
      <form class="form-signin" method="POST" action="<?php echo $_SERVER['PHP_SELF']?>">
        ...
        <input style="background-color:yellow;" type="text" name="contest" class="input-block-level" placeholder="Kì thi" <?php if (isset($_GET['kt'])) echo "value='".$_GET['kt']."'";?>>
	...
	<center>thnam @ thptccva.edu.vn</center>
    </div> <!-- /container -->
