...
<?php
	if (isset($success) && $success) {
		if (!is_file($contestDir . 'tmp.xml')) {
			$fi = fopen($contestDir . 'account.xml','r');
			$fo = fopen($contestDir . 'tmp.xml','w');
			while (!feof($fi)) {
				$str = fgets($fi);
			...
			fclose($fi);
			fclose($fo);
			copy($contestDir . 'tmp.xml',$contestDir . 'account.xml');
			unlink($contestDir . 'tmp.xml');
